...
NGINXCONF = {'/etc/nginx/nginx.conf', '/usr/local/nginx/conf/nginx.conf'}
...
def loadconfig(conf=None, getlineinfo=False):
    ...
    p = subprocess.Popen(shlex.split("nginx -t"), stdout=subprocess.PIPE, close_fds=True)
    res = p.stdout.read()

    print '----', res, res.splitlines(False)
    # for line_i, line in res.splitlines():
    #     print line_i, line
    ...
    # print '----',_conf
    return _loadconfig(_conf, getlineinfo)
...
if __name__ == '__main__':
    ...
    print loadconfig(NGINXCONF, True)
